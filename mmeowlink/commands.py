...
class TempBasal(PumpCommand):
    ...
    def Program(klass, rate=None, duration=None, temp=None, **kwds):
        ...
        assert temp in ['percent', 'absolute'], "temp field <{0}> should be one of {1}".format(temp,
                                                                                               ['percent', 'absolute'])
        if temp in ['percent']:
...
class ReadHistoryData(PumpCommand):
    ...
    _test_ok = bytearray(
        [0x01, 0x00, 0xA7, 0x01, 0x20, 0x88, 0x50, 0x80, 0x01, 0x00, 0x02, 0x02, 0x00, 0x80, 0x9B, 0x03, 0x36, ])
    ...
    def respond(self, raw):
        ...
        if len(raw) == self.size:
            log.info(
                '{} download respond replace original {} with found {}'.format(str(self), len(self.data), len(raw)))
            self.data = raw
